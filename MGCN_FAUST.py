...
from datasets.faust_wavelet import FAUST_WAVELET
...
parser.add_argument('--saving_name', '--sn', default='mgcn_faust', type=str,
                    help='the name of trained models and the name of directory to save output descriptors')
parser.add_argument('--loading_name', '--ln', default='mgcn_faust-300', type=str,
    ...
    if TRIPLET:
        ...
        for data in train_loader_tri:
            if len(data)==2:
                ...
                out_a = model(data_a.to(device))[1][0:d.num_nodes, :]
                out_p = model(data_p.to(device))[1][0:d.num_nodes, :]
                optimizer.zero_grad()
...
for epoch in range(1, EPOCH+1):
    if GEN:
        ...
        txt_path = osp.join(path_gen, 'tr_reg_{0:03d}.txt')
    ...
    if epoch % args.save_freq == 0:
        ...
        txt_path = osp.join(osp.abspath('.'), 'outputs', SAVE_NAME, 'tr_reg_{0:03d}.txt')
        for data in test_loader:
            desc = model(data.to(device))[1]
            descriptor = desc.cpu().detach().numpy()
            i = int(data.name.cpu().detach())
